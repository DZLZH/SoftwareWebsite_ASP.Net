<%@ Page Language="C#" AutoEventWireup="true" CodeFile="filelist.aspx.cs" Inherits="manger_downfile_filelist" %>
...
<table width="100%" align="center" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF" class="newtable">
      <tr>
        ...
        <input id="Button1" type="button" value="添加文件" style="height:25px" onclick="location='addfile.aspx';" />&nbsp;&nbsp;&nbsp;
            &nbsp;
	  ...
	  <tr onmouseover="this.style.backgroundColor='#e1e5ee';" onmouseout="this.style.backgroundColor='';">
		<td align="left">&nbsp;<%#Eval("down_name")%></td>
		<td align="center">&nbsp;<%#Eval("down_createtime")%></td>
		<td align="center">&nbsp;<asp:LinkButton ID="lbtshenhe" Text='<%#Getshenhe(Eval("down_shenhe"))%>' ForeColor='<%#GetshenheColor(Eval("down_shenhe"))%>' runat="server" CommandName='<%#Eval("down_shenhe") %>' CommandArgument='<%#Eval("down_id") %>'></asp:LinkButton></td>
		<td align="center">&nbsp;<asp:LinkButton ID="LinkButton2" Text='<%#Getrem(Eval("down_recommand"))%>' ForeColor='<%#GetremColor(Eval("down_recommand"))%>' runat="server" CommandName='rem' CommandArgument='<%#Eval("down_id") %>'></asp:LinkButton></td>
		<td align="center">
		<a href="addfile.aspx?id=<%#Eval("down_id") %>">修改</a> | <asp:LinkButton ID="LinkButton1" runat="server" CommandName="delete" CommandArgument='<%#Eval("down_id") %>' OnClientClick="return confirm('确认删除吗?')">删除</asp:LinkButton>
		</td>
	  </tr>
	  </ItemTemplate>
      </asp:Repeater>
